...
/**
 * Base copy edit policy for copy/cut action.
 * 
 * @since 1.2.0
 */
public class DefaultCopyEditPolicy extends AbstractEditPolicy {
	...
	private Command getCopyCommand(Request request) {
		...
		return new Command() {
			...
			public void execute() {
				...
				Resource newEMFResource = javaBeanToCopy.eResource().getResourceSet().createResource(
						javaBeanToCopy.eResource().getURI().appendSegment("" + javaBeanToCopy.hashCode())); //$NON-NLS-1$
	...
	protected void preExpand(final IJavaInstance javaBean) {
		IJavaInstance beanCopy = (IJavaInstance) copier.get(javaBean);
		FactoryCreationData.contributeToCopy(javaBean, beanCopy, domain);
	}
}