 ...
 */
public abstract class XYLayoutEditPolicy extends org.eclipse.gef.editpolicies.XYLayoutEditPolicy implements IZoomListener, IGridListener,
		IActionFilter {
	...
	/**
	 * If gridding is not wanted, then set this to false. The default is true.
	 */
	...
	/**
	 * If zooming is wanted, then set this to true. The default is false;
	 */
	...
	/*
	 * Get grid state data from the edit domain to determine whether to turn on/off the grid. The state is set each the time the grid is turned on or
	 * off. We need this in order to reshow the grid in case a reload from scratch occurs. The data is a HashSet with the annotation name as the key
	 */
	...
	/**
	 * The child's constraint is being changed. This shouldn't be called anymore. All now go through getResizeChildrenCommand and test and set the
	 * ismove/isresize appropriately.
	 */
	...
	/**
	 * Create the Primary Drag Role Edit Policy. This handles only the selection handles and drag feedback.
	 */
	protected EditPolicy createChildEditPolicy(final EditPart aPart) {
		...
		EditPolicy dragRolePolicy = isChildResizeable(aPart) ? new org.eclipse.gef.editpolicies.ResizableEditPolicy()
				: new org.eclipse.gef.editpolicies.NonResizableEditPolicy();
		return new PrimaryDragRoleEditPolicy(dragRolePolicy, true);
	}

	/**
	 * Called when the context menu is about to open. Override to add your own context dependent menu contributions to a child view object.
	 */
	...
	protected Command getAlignmentChildCommand(Request request) {
		...
		Command cmd = createChangeConstraintCommand(child, translateToModelConstraint(newRect), isAlignmentMove(alignReq.getAlignType()),
				isAlignmentResize(alignReq.getAlignType()));
		return cmd != null ? cmd : NoOpCommand.INSTANCE; // If no changes (i.e. null returned) we still must return something to indicate we
	...
	/*
	 * Get the constraint from the super class, and adjust it's position relative to it's host position, and removing the zoom factor from and it's
	 * position.
	 */
	...
	/*
	 * Create a figure used for target feedback when dragging. Use the rectangle parameter as it's bounds.
	 */
	...
	/**
	 * Convert the model constraint into a draw2d point or rectangle. This is different than {@link #modelToFigureConstraint(Object)} because
	 * this just does a straight convert so that we can work with the coordinates from a common class. The modelToFigure is used to convert from
	 * a figure constraint to a model constraint, and this could include some translations or scaling. So this method does none of that. It just
	 * gets the models point or rectangle as needed.
	 * 
	 * @param modelconstraint
	 * @return draw2d point or rectangle.
	 * 
	 * @since 1.2.0
	 */
	protected abstract Object convertModelToDraw2D(Object modelconstraint);

	protected void showCursorFeedback(Request aRequest) {
		Point position = getLocationFromRequest(aRequest).getCopy();	// This is absolute to the current displayed window, not to the figure.
		...
		cursorLabel.setText(MessageFormat.format(CDEMessages.XYLayoutEditPolicy_CursorFeedback_X_Y, new Object[] { String.valueOf(position.x),
	...
	/**
	 * Returns a draw2d constraint object for the given request. The returned object can be translated to the model using
	 ...
	 * 
	 * Return true for show grid action if grid is hidden Return true for hide grid action if grid is showing otherwise return false
	 */
