 ...
 * from the Graphviewer, we depend on the tree editpart for selecting the
 * pages or components on those pages. 
 * 
 * When a page (tree editpart.. child of JTabbePane) is selected,
 * the corresponding JTabbedPaneGraphicalEditPart is selected and it 
 * notifies the JTabbedPanedProxyAdapter to bring the page to the front.
 * 
 * When this occurs, the previously graph viewed page figures are hidden and the 
...
// keep the following for the future so we can show the tab title in the beans viewer.		
	...
	protected void createEditPolicies() {
		...
		installEditPolicy(
			...
			new org.eclipse.ve.internal.cde.core.TreeContainerEditPolicy(new JTabbedPaneContainerPolicy(EditDomain.getEditDomain(this))));
	}
