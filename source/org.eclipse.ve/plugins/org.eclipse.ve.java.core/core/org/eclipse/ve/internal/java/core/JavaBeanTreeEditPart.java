...
public class JavaBeanTreeEditPart extends DefaultTreeEditPart implements IJavaBeanTreeContextMenuContributor {
	...
	protected Adapter getListenerAdapter() {
		if (listenerAdapter == null) {
			listenerAdapter = new Adapter() {
				public void notifyChanged(Notification notification) {
					if (notification.getFeature() == getSFObjectEvents()) {
						// When we refresh our entire children we should remove any listeners that we have on the eventInvocation instances
	...
	public final List getModelChildren() {
		...
		if(getParent() == null) return Collections.EMPTY_LIST;
		...
		if ( intShowEvents  != JavaVEPlugin.EVENTS_NONE) {
			...
			// is to make sure that someone doesn't accidently forget to include the event child and it also lets us change how events are added in a central way
	...
	/**
	 * Return the existing events for our model.  This is the relationship Object/events and will be a list
	 * of Events.xmi/EventInvocation objects 
	 */
	...
	protected Adapter getEventInvocationAdapter() {
		if (eventInvocationAdapter == null) {
			eventInvocationAdapter = new Adapter() {
				public void notifyChanged(Notification notification) {
					// TODO needs changing to some more efficient code that Rich has
	...
	protected Adapter getPropertyChangeEventInvocationAdapter() {
		if (propertyChangeEventInvocationAdapter == null) {
			propertyChangeEventInvocationAdapter = new Adapter() {
				public void notifyChanged(Notification notification) {
	...
	protected List getBasicEvents(List eventInvocations) {
		...
		while (iter.hasNext()) {
			...
			if ( eventInvocation instanceof EventInvocation ) {
				...
				if (propertyChangeEventInvocationsListenedTo == null) propertyChangeEventInvocationsListenedTo = new ArrayList(1);
		...
		Collections.sort(result,new Comparator() {
			...
			public int compare(Object o1, Object o2) {
				...
				if (o1 instanceof EventCallback) {
					if (o2 instanceof PropertyEvent ) return -1;
						return coll.compare(((EventCallback)o1).getCallback().getMethod().getName(), ((EventCallback)o2).getCallback().getMethod().getName());						
				} else {
	...
	protected List getExpertEvents(List eventInvocations) {
		...
		if (eventInvocations.isEmpty()) {
			...
			// the child in the tree is the actual listener itself - and its children will represent each of the JavaEvents that we use to point to it
			Iterator iter = eventInvocations.iterator();
			while (iter.hasNext()) {
				AbstractEventInvocation eventInvocation = (AbstractEventInvocation) iter.next();
	...
	class DeleteEventCallbackCommand extends Command{
		...
		public void execute() {
			//TODO Rich is going to get upset if I don't do this properly using command builders - Need to change later, including all of the other event command execute() methods, JRW
