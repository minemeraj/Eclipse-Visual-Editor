 ...
 *******************************************************************************/
/*
 *  $RCSfile: BorderLayoutLayoutPage.java,v $
 *  $Revision: 1.1 $  $Date: 2006-02-19 14:09:11 $ 
 */
 ...
 * @since 1.2.0
 */
public class BorderLayoutLayoutPage extends JavaBeanCustomizeLayoutPage {
	...
	protected boolean handleSelectionChanged(ISelection oldSelection) {
		...
		if (newSelection != null && newSelection instanceof IStructuredSelection && !((IStructuredSelection) newSelection).isEmpty()) {
			...
			if (editparts.size() == 1 && editparts.get(0) instanceof EditPart) {
				...
				// check to see if this is a container with a BorderLayout
			...
			// Else check to see if the parent of all the selected components is a border layout
			if (editparts.get(0) instanceof EditPart && ((EditPart) editparts.get(0)).getParent() != null) {
				...
				// Check the parent to ensure its layout policy is a BorderLayout
				if (isValidTarget(firstParent)) {
					 ...
					 * - it's parent has a BorderLayout as it's layout manager
					 */
	...
	/*
	 * Return true if the parent's layout policy is a BorderLayout.
	...
	public boolean isValidTarget(EditPart target) {
		...
		if (af != null && af.testAttribute(target, LAYOUT_FILTER_KEY, BorderLayoutEditPolicy.LAYOUT_ID)) {
			return true;
		}
		return false;
	}
	...
	protected ResourceSet getResourceSet(EditPart editpart) {
		if (rset == null) {
			...
			sfHgap = JavaInstantiation.getSFeature(rset, JFCConstants.SF_BORDERLAYOUT_HGAP);
			sfVgap = JavaInstantiation.getSFeature(rset, JFCConstants.SF_BORDERLAYOUT_VGAP);
		}
		return rset;
	}
