    ...
    <?php            
       ...
       while ($testinfo = fscanf ($f, "%s\t%s\t%s\t%s\t%[a-zA-Z0-9,. \\'';;~~!!@@##$$%%&&**(())--==++__]\n")) {
              ...
              if (($tst == $_POST["Test"]) && ($tester == "None") && ($a!="") && ($b=="")) {
                  $tester = $_POST["Email"];                  
                  // ve-dev@eclipse.org 
//                  mail("walkerp@us.ibm.com, " . $_POST["Email"], $_POST["Test"] . " assigned to " . $_POST["Email"], "VisualEditor, 1.2 M1 TestPass\n\tTestcase " . $_POST["Test"] . " was assigned to ". $_POST["Email"].".", "From: VEproject@{$_SERVER['SERVER_NAME']}");                  
                  mail("ve-dev@eclipse.org, " . $_POST["Email"], $_POST["Test"] . " assigned to " . $_POST["Email"], "VisualEditor, 1.2 M1 TestPass\n\tTestcase " . $_POST["Test"] . " was assigned to ". $_POST["Email"].".", "From: gmendel@us.ibm.com");                  
              }
       ...
       fclose($f2);
       unlink($datafile);
//       echo "<h2>replacing tests-ve1.2.txt.new>tests-ve1.2.txt</h2><br>";
...
//       echo "removing $newdatafile <br>";
       ...
       if(!unlink($newdatafile))
       	exit("unable to delete tests-ve1.2.txt.new");
//       echo "$newdatafile exists? " . file_exists($newdatafile) . "<br>";
