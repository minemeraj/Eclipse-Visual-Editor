...
public IPointBeanProxy createPointBeanProxyWith(int x, int y) {
	try {
		return (IPointBeanProxy) fBeanTypeFactory.getBeanTypeProxy("org.eclipse.swt.graphics.Point").newInstance("new org.eclipse.swt.graphics.Point("+x+","+y+")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
...
public IRectangleBeanProxy createBeanProxyWith(int x, int y, int width, int height) {
	try {
		return (IRectangleBeanProxy) fBeanTypeFactory.getBeanTypeProxy("org.eclipse.swt.graphics.Rectangle").newInstance("new org.eclipse.swt.graphics.Rectangle("+x+","+y+","+width+","+height+")"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
	} catch (ThrowableProxy e) {
		return null;
	} catch (InstantiationException e) {
		return null;	// Shouldn't occur
