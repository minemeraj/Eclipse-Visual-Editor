 ...
 *******************************************************************************/
/*
 * $RCSfile: EventInvocationAndListenerTreeEditPart.java,v $ $Revision: 1.5 $ $Date: 2004-03-26 23:08:01 $
 */
		...
		eventInvocationAndListener.addNotifier(new EventInvocationAndListener.Observer() {
			...
			public void eventInvocationChanged() {
				...
				if (getParent() != null && getWidget() != null) {
					CDEUtilities.displayExec(EventInvocationAndListenerTreeEditPart.this, new Runnable() {
						public void run() {
							if (isActive()) {
								refreshVisuals();
								refreshChildren();
