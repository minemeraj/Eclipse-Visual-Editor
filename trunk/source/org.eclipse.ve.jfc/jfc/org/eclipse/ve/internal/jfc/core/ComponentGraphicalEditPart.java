 ...
 *******************************************************************************/
/*
 *  $RCSfile: ComponentGraphicalEditPart.java,v $
...
/**
 * EditPart for a java.awt.Component.
 * The parent editpart is responsible to set transparent.
 * If transparent, then there won't be any image capture.
 *
 * Note: Bit of a kludge, but because a component could be
 * either on the freeform or within another component, we
 * need to differentiate for propertySource between the
 * component and the componentConstraint (or whatever).
 * This is so that when on the freeform, the component property source
 * is it, but when on another component, we want to merge the
 * constraint info with the component features. This is done by
 * setting into the editpart who should be used as the model to find the propertySource.
 * The default is the model of the editpart, while if set, should use the propertySourceModel.
 * The component itself is considered to be the model. This is so that when moving a component
 * between two different parents, it will see the component and not the constraint as to what
 * is being added. The policy will create a new constraint for where it is going.
 * 
 * The initialization data is used to configure whether this edit part should show borders on
 * the figure or not. The default is false. The initialization data can be "true" or "false",
 * case-insensitive. This init data can be placed in the xmi to determine what it should be.
 */
...
/**
 * If we are transparent, then have the proxy set to don't apply visible. If on the
 * freeform (i.e. not transparent) we want it to always be visible.
 */
...
protected void createEditPolicies() {
	...
	if (getParent() instanceof ContentsGraphicalEditPart) {
		// TODO At the moment a bit of kludge to get the FreeForm Dialogs working.
...
private void performDirectEdit(){
	if(manager == null)
		manager = new ComponentDirectEditManager(this, 
			TextCellEditor.class, new ComponentCellEditorLocator(getFigure()), sfDirectEditProperty);
	manager.show();
}
...
/**
 * Return the ComponentProxy.
 * Creation date: (3/15/00 12:33:41 PM)
...
/**
 * Parents of this will call this and pass in true
 * if the image should be retrieved. It is important that
 * this is called by parent editpart before the
 * editpart is activated (this is best done in 
 * the createEditPart method of the parent edit part).
 */
...
protected IVisualComponent getVisualComponent() {
	return (IVisualComponent) getComponentProxy(); // For AWT, the component proxy is the visual component.
}
	/**
	 * Return a list of edit policies.
	 * 
	 * This should not be needed but GEF doesn't make the edit policies public
	 * except by a specific key.
	 */
