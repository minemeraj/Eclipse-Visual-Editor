...
public class SingleImageDisplay extends Canvas{
    ...
    private void addListeners(){
        ...
        addDisposeListener(new DisposeListener() {
            public void widgetDisposed(DisposeEvent e) {
                if( img != null && img != ImageController.warnIcon && img != ImageController.errorIcon && img != ImageController.noMemoryIcon ){
                    img.dispose();
                    img = null;
                }
            }
    ...
    public void setImage(final IPath path, final IWorkspaceRoot wsroot){
		...
		warningMsg = null;
    	if (img != null && img != ImageController.noMemoryIcon && img != ImageController.warnIcon && img != ImageController.errorIcon) {
    		img.dispose();
    		img = null;
    	}
    	...
    	if (filename != null) {
    		try {
    			...
    			if (warningMsg == null) {
    				...
    				warningMsg = SWTMessages.getString("SingleImageDisplay.ImageNotLoaded_WARN_"); //$NON-NLS-1$
    				JavaVEPlugin.log(e);	// Log it so we can go back and look
    			}
    			img = ImageController.warnIcon;
    		} catch (OutOfMemoryError e) {
    			warningMsg = SWTMessages.getString("SingleImageDisplay.OutOfMemory_WARN_"); //$NON-NLS-1$
    			img = ImageController.noMemoryIcon;
    		}
    	}
