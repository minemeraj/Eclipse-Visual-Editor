...
public class IconProxyAdapter extends BeanProxyAdapter {
	EditDomain fEditDomain;
	/**
	 * Constructor for IconProxyAdapter.
	 * @param domain
	 */
		...
		if (index != -1) {
			...
			if (firstQuoteIndex != -1) {
				...
				if (lastQuoteIndex != -1) {
					...
					try {
						IProject project =
							((FileEditorInput) (fEditDomain).getEditorPart().getEditorInput()).getFile().getProject();
					...
					if (file != null) {
						...
						// Prepend the beginning part (typically new javax.swing.ImageIcon(" to the new
						...
						initString =
							initString.substring(0, index)
								+ BeanUtilities.createStringInitString(fullPath)
								+ initString.substring(lastQuoteIndex + 2);

					} else {
						throw new InstantiationException(
							MessageFormat.format(VisualMessages.getString("IconProxyAdapter.CannotReadFileError_EXC_"), new Object[] { pathString })); //$NON-NLS-1$
					}
				}
			}
		} else {
			// This is just a file name inside a quoted string. Check to see if the file exists.
			String filename = ""; //$NON-NLS-1$
			int firstQuoteIndex = initString.indexOf('"'); //$NON-NLS-1$
			if (firstQuoteIndex != -1) {
				int lastQuoteIndex = initString.indexOf('"', firstQuoteIndex + 1); //$NON-NLS-1$
				if (lastQuoteIndex != -1) {
				...
				if (filename.length() > 0) {
					...
					if (!iconFile.canRead()) {
						throw new InstantiationException(
							MessageFormat.format(VisualMessages.getString("IconProxyAdapter.CannotReadFileError_EXC_"), new Object[] { filename })); //$NON-NLS-1$
					}
				}
			}
		}
