 ...
 *******************************************************************************/
/*
 *  $RCSfile: FlowLayoutLayoutPage.java,v $
 *  $Revision: 1.1 $  $Date: 2006-02-19 01:32:34 $ 
 */
...
public class FlowLayoutLayoutPage extends JavaBeanCustomizeLayoutPage {
	...
	EStructuralFeature sfAlignment, sfHgap, sfVgap;
	
	boolean initialized = false;
	
	boolean allEnabled;
	
	private static final String[] alignmentInitStrings = new String[] {
		"java.awt.FlowLayout.LEFT", //$NON-NLS-1$
		"java.awt.FlowLayout.CENTER", //$NON-NLS-1$
		"java.awt.FlowLayout.RIGHT" //$NON-NLS-1$
	};
	...
	public Control getControl(Composite parent) {
		...
		SelectionListener radioSelection = new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				radioModified((Button)e.widget);
			}
		};
		
		Group alignmentGroup = new Group(c, SWT.NONE);
		alignmentGroup.setText(JFCMessages.FlowLayoutLayoutPage_AlignmentGroupTitle); 
		GridData gd = new GridData();
		gd.verticalAlignment = GridData.BEGINNING;
		gd.horizontalAlignment = GridData.FILL;
		alignmentGroup.setLayoutData(gd);
		GridLayout grid = new GridLayout();
		alignmentGroup.setLayout(grid);
		
		alignmentLeftRadio = new Button(alignmentGroup, SWT.RADIO);
		alignmentLeftRadio.setText(JFCMessages.FlowLayoutLayoutPage_LeftAlign);
		alignmentLeftRadio.addSelectionListener(radioSelection);
		
		alignmentCenterRadio = new Button(alignmentGroup, SWT.RADIO);
		alignmentCenterRadio.setText(JFCMessages.FlowLayoutLayoutPage_CenterAlign);
		alignmentCenterRadio.addSelectionListener(radioSelection);
		
		alignmentRightRadio = new Button(alignmentGroup, SWT.RADIO);
		alignmentRightRadio.setText(JFCMessages.FlowLayoutLayoutPage_RightAlign);
		alignmentRightRadio.addSelectionListener(radioSelection);
	...
	protected boolean handleSelectionChanged(ISelection oldSelection) {
		...
		if (newSelection != null && newSelection instanceof IStructuredSelection && !((IStructuredSelection) newSelection).isEmpty()) {
			...
			if (editparts.size() == 1 && editparts.get(0) instanceof EditPart) {
				...
				// check to see if this is a container with a FlowLayout
			...
			// Else check to see if the parent of all the selected components is a flow layout
			if (editparts.get(0) instanceof EditPart && ((EditPart) editparts.get(0)).getParent() != null) {
				...
				// Check the parent to ensure its layout policy is a FlowLayout
				if (isValidTarget(firstParent)) {
					 ...
					 * - it's parent has a FlowLayout as it's layout manager
					 */
	...
	public boolean isValidTarget(EditPart target) {
		...
		if (af != null && af.testAttribute(target, LAYOUT_FILTER_KEY, FlowLayoutEditPolicy.LAYOUT_ID)) { //$NON-NLS-1$ //$NON-NLS-2$
			return true;
		}
		return false;
	}
	...
	private void initializeValues() {
		...
		int alignment = getIntValue(fEditPart, sfAlignment);
		switch (alignment)
		{
			case LEFT :
				alignmentLeftRadio.setSelection(true);
				alignmentCenterRadio.setSelection(false);
				alignmentRightRadio.setSelection(false);
				break;
			case CENTER :
				alignmentLeftRadio.setSelection(false);
				alignmentCenterRadio.setSelection(true);
				alignmentRightRadio.setSelection(false);
				break;
			case RIGHT :
				alignmentLeftRadio.setSelection(false);
				alignmentCenterRadio.setSelection(false);
				alignmentRightRadio.setSelection(true);
				break;
		}
	...
	private void radioModified(Button b) {
		if (initialized) {
			int alignment;
			if (b == alignmentLeftRadio) {
				alignment = LEFT;
			} else if (b == alignmentCenterRadio) {
				alignment = CENTER;
			} else {
				alignment = RIGHT;
			}
				
			execute(createAlignmentCommand(fEditPart, alignment));
		}
	}
	...
	protected Command createAlignmentCommand(EditPart editpart, int alignment) {
		...
		if (control != null) {
			...
			if (layout != null) {
				...
				String init = alignmentInitStrings[alignment];
				Object intObject = BeanUtilities.createJavaObject("int", rset, init); //$NON-NLS-1$
				componentCB.applyAttributeSetting(layout, sfAlignment, intObject);
	...
	protected Command createSpinnerCommand(EditPart editpart, EStructuralFeature sf, Spinner spinner) {
		...
		EObject control = (EObject)editpart.getModel();
		if (control != null) {
			IJavaInstance flowLayout = (IJavaInstance) control.eGet(sfCompositeLayout);
			if (flowLayout != null) {
				RuledCommandBuilder componentCB = new RuledCommandBuilder(EditDomain.getEditDomain(editpart), null, false);
				String init = String.valueOf(spinner.getSelection());
				Object intObject = BeanUtilities.createJavaObject("int", rset, init); //$NON-NLS-1$
				componentCB.applyAttributeSetting(flowLayout, sf, intObject);
				componentCB.applyAttributeSetting(control, sfCompositeLayout, flowLayout);
				cb.append(componentCB.getCommand());
			} else {
				// this shouldn't happen
				return UnexecutableCommand.INSTANCE;
	...
	protected int getIntValue(EditPart ep, EStructuralFeature sf) {
		...
		if (ps != null && getResourceSet(ep) != null) {
			IPropertySource flowLayout = (IPropertySource) ps.getPropertyValue(sfCompositeLayout);
			if (flowLayout != null) {
				Object intPV = flowLayout.getPropertyValue(sf);
	...
	protected ResourceSet getResourceSet(EditPart editpart) {
		if (rset == null) {
			...
			sfAlignment = JavaInstantiation.getSFeature(rset, JFCConstants.SF_FLOWLAYOUT_ALIGNMENT);
			sfHgap = JavaInstantiation.getSFeature(rset, JFCConstants.SF_FLOWLAYOUT_HGAP);
			sfVgap = JavaInstantiation.getSFeature(rset, JFCConstants.SF_FLOWLAYOUT_VGAP);
		}
		return rset;
	}
