...
/**
 * This property editor is used for the constraints attribute for components whose parent container
 * has a layout manager of java.awt.BorderLayout
 * It is an example of a property editor for beans that is specialized and is part of the IDE
 * The IDE knows to use it because the BeanType for the BorderLayout bean class has the
 * constraintsAttributePropertyEditorClass set to the name of this class
 * It could also be done by specifying a java.beans.PropertyEditor as well by setting the atttribute
 * constraintsAttributeJavaBeansPropertyEditorClassName
 * Creation date: (1/27/00 6:50:03 AM)
...
/** 
 * The value passed in is actually the MOFObject that wrappers the
...
protected void doSetObject(Object value) {
	if (isValueValid() && value instanceof IJavaObjectInstance) {
		...
		fCurrentStringConstraint = (String)BorderLayoutPolicyHelper.DISPLAY_TAGS.get(BorderLayoutPolicyHelper.REAL_INTERNAL_TAGS.indexOf(constraintProxy.stringValue()));
	}
	// Get the component orientation from the container (firstConstraintComponent is the ConstraintComponent itself, so it's container is the awt.Container).
...
protected int doGetIndex(Object value){
	...
	for(int i=0;i<getTags().length;i++){
		if(fTags[i].equals(fCurrentStringConstraint)){
			return i;
		}
	}	
	return NO_SELECTION;
 ...
 * To do this traverse from the component ( our source ) to its parent container
 * and then collect a list of all of the components of the container.
...
/**
 * Return the list of allowable tags for border layout
 * These are cached in fTags and calculated lazily
 * The tags are the unoccopied constraints.  To see which are unoccupied we must get the occupied
 * constraints and then remove these from the list of occupied ones, adding back in our own constraint
 * ,e.g. the getOccupiedConstraints() could return "North" "Center" "East" and we then create a list
 * that includes "West" and "South" because these are valid and available.  However our own current
 * constraint ( e.g. "North" ) is also available so we add it back in
 */
